...
![Commit Activity](https://img.shields.io/github/commit-activity/m/Pentatrate/beattools)
![Commits Since Release](https://img.shields.io/github/commits-since/Pentatrate/beattools/latest)
...
(And is also availiable for the Demo version of the game! Check out the post in `demo-mods` with its original name, `Editor Overhaul`)
...
2. Unzip and place in `beatblock/Mods/` folder (where all the other mods are)
	...
	**!!!Reminder!!!**: Make sure the mod.json file is in `beatblock/Mods/beattools/mod.json` and **NOT** `beatblock/Mods/beattools/beattools/mod.json`
