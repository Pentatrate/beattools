...
function tooly.addFuncsToTunnels(tunnels, funcs, a1)
	...
	if funcs[1] then
		local added = false
		for _, tunnel in ipairs(returnTunnels) do


			if intersection.isTimeOverlapping(tunnel, fake) then
				local funcs2 = helpers.copy(funcs)
				funcs2 = intersection.remove(funcs2, nil, tunnel.startTime)
				funcs2 = intersection.remove(funcs2, tunnel.endTime, nil)

				if a1 ~= false then
					tunnel.a1 = intersection.addFunctions(tunnel.a1, funcs2)
				end
				if a1 ~= true then
					tunnel.a2 = intersection.addFunctions(tunnel.a2, funcs2)
				end
				added = true
			end
		end
...
function tooly.getTunnelsForEventAndPaddle(event, paddleId)
	...
	if startCount.newWidth.total == 0 then
		...
		while startCount.newWidth.index <= i and i <= endCount.newWidth.index and list[i] and list[i].event.time < tunnels[#tunnels].endTime do -- a few double checks dont hurt
			...
			if i + 1 > endCount.newWidth.index or not list[i + 1] or list[i + 1].event.time ~= ease.time then
				if time < ease.time then
					...
					local value = list[i - 1] and list[i - 1].event or utilitools.files.beattools.easing.getEase("paddles", paddleId, ease.time, ease.order, utilitools.files.beattools.easing.getIndex(ease) - 1)
				...
				if ease.duration and ease.duration ~= 0 and ease.time + ease.duration > globalStart then
					...
					if false and ease.start then -- paddles events dont have a start, why did i do this
						...
						local value = list[i - 1] and list[i - 1].event.time + (list[i - 1].event.duration or 0) <= ease.time and list[i - 1].event or utilitools.files.beattools.easing.getEase("paddles", paddleId, ease.time, ease.order, utilitools.files.beattools.easing.getIndex(ease) - 1)
		...
		if time < globalEnd then
			...
			local value = list[i - 1] and list[i - 1].event or utilitools.files.beattools.easing.getEase("paddles", paddleId, time, nil, nil)
	...
	if startCount2.total == 0 then
		...
		while startCount2.index <= i and i <= endCount2.index and list[i] and list[i].event.time < tunnels[#tunnels].endTime do -- a few double checks dont hurt
			...
			if i + 1 > endCount2.index or not list[i + 1] or list[i + 1].event.time ~= ease.time then
				if time < ease.time then
					-- the paddle width before the ease
					local value = list[i - 1] and list[i - 1].event or utilitools.files.beattools.easing.getEase("ease", "objectRotation", time, nil, nil)
				...
				if ease.duration and ease.duration ~= 0 and ease.time + ease.duration > globalStart then
					...
					if ease.start then
						...
						local value = list[i - 1] and list[i - 1].event.time + (list[i - 1].event.duration or 0) <= ease.time and list[i - 1].event or utilitools.files.beattools.easing.getEase("ease", "objectRotation", ease.time, ease.order, utilitools.files.beattools.easing.getIndex(ease) - 1)
...
function tooly.tunnelGetRange(tunnel, time) -- returns ranges, not range, since its easier that way
	...
	local ranges = { { a1 % 360, a2 % 360 } }
	--[[ if math.abs(ranges[1][1] - ranges[1][2]) <= 2 * 1e-4 then
		ranges[1][1] = ranges[1][1] - 1e-4
		ranges[1][2] = ranges[1][2] + 1e-4
	end ]]
	return ranges
end
	...
	while tunnels[i] do
		...
		while tunnels[j] do
			...
			if tunnelA ~= tunnelB and tunnelA.endTime == tunnelB.startTime then
				...
				if diff1 <= 1e-8 and diff2 <= 1e-8 then
...
function tooly.mergeTunnel(tunnel1, tunnel2, canBeEither, onlyOverlap) -- no longer splits the tunnels
	local intersection = utilitools.files.beattools.intersection
	if not intersection.isTimeOverlapping(tunnel1, tunnel2) then
...
function tooly.tunnelsGetRanges(tunnels, antiTunnels, time, notTunnel, printIt)
	local intersection = utilitools.files.beattools.intersection
	local ranges
	local startRanges
	local endRanges
	...
	if tunnels then
		for _, tunnel in ipairs(tunnels) do
			if tunnel ~= notTunnel and intersection.inTime(tunnel, time) then
				local moreRanges = tooly.tunnelGetRange(tunnel, time)
				if moreRanges then
					if tunnel.startTime == time then
						startRanges = tooly.overlapRanges(startRanges or {}, moreRanges, true)
					elseif tunnel.endTime == time then
						endRanges = tooly.overlapRanges(endRanges or {}, moreRanges, true)
					else
						ranges = tooly.overlapRanges(ranges or {}, moreRanges, true)
					end
				end
			end
		end
	...
	if antiTunnels then
		for _, tunnel in ipairs(antiTunnels) do
			if intersection.inTime(tunnel, time) then
				local moreRanges = tooly.tunnelGetRange(tunnel, time)
				antiRanges = tooly.overlapRanges(antiRanges, moreRanges, false)
			end
		end
	end
	finalRanges = tooly.overlapRanges(finalRanges, startRanges, false)
	finalRanges = tooly.overlapRanges(finalRanges, endRanges, false)
	if finalRanges or ranges then
		finalRanges = tooly.overlapRanges(finalRanges or {}, ranges or {}, true)
	else
		finalRanges = tooly.overlapRanges(finalRanges, ranges, false)
	end
	finalRanges = tooly.overlapRanges(finalRanges, antiRanges, false)
	if printIt then modlog(mod, "\n\t\t\t\t\ttunnel ranges", time, startRanges, endRanges, ranges, antiRanges, finalRanges, "e") end
	return finalRanges
end
...
function tooly.getRangesBetween()
	...
	if tooly.allEvents and #tooly.allEvents > 0 then -- tunnels
		...
		for i, event in ipairs(tooly.allEvents) do
			doProgress(i)
			local eventTime = event.time
			if event.type == "mineHold" and (event.duration or 1) ~= 0 and (not minTime or minTime <= eventTime) and (not maxTime or eventTime <= maxTime) then
	...
	if tooly.data.tunnels and #tooly.data.tunnels > 0 then -- culling
		...
		while tooly.data.tunnels[i] do
			...
			local function checkRanges(time, notTunnel)
				local ranges
				if notTunnel then
					ranges = tooly.tunnelsGetRanges(tooly.data.tunnels, tooly.data.antiTunnels, time, notTunnel)
				else
					ranges = tooly.getRangesForTime(time, tooly.data.tunnels, tooly.data.antiTunnels)
				end
				local rangesTunnel = tooly.tunnelGetRange(tunnel, time)
				local result = tooly.overlapRanges(ranges, rangesTunnel, false)
				if not result or #result ~= 0 then
					if notTunnel then
						tooly.tunnelsGetRanges(tooly.data.tunnels, tooly.data.antiTunnels, time, notTunnel, true)
						modlog(mod, "\n\t\t\t\tresult true", utilitools.string.concat(ranges), rangesTunnel, result)
					end
					return true
				else
					return false
			...
			if diff <= 1e-4 then
				success = checkRanges(tunnel.startTime, tunnel)
				if success then modlog(mod, "\n\t\t\t\tstill start success", tunnel.startTime) end
			end
			if success then
				diff = math.abs(intersection.useFuncs(tunnel.a1, tunnel.endTime) - intersection.useFuncs(tunnel.a2, tunnel.endTime)) % 360
				if diff > 180 then diff = math.abs(diff - 360) end
				if diff <= 1e-4 then
					success = checkRanges(tunnel.endTime, tunnel)
					if success then modlog(mod, "\n\t\t\t\tstill end success", tunnel.endTime) end
				end
			end
	...
	if tooly.data.times and #tooly.data.times > 0 then -- pathing
		...
		for i, time in ipairs(tooly.data.times) do
			...
			local function sortRanges()
				if not ranges[1] then modwarn(mod, "IMPOSSIBLE", time, prevAngle, ranges, inTunnels) return prevAngle end
			...
			if i ~= 1 then
				...
				if duration > 1e-8 then
					...
					local valid, reason = intersection.validateFunctions(path, true, true)
					if not valid then
						modlog(mod, reason, prevTime, duration, prevAngle, nextAngle)
					end
		...
		local valid, reason = intersection.validateFunctions(tooly.data.path.path, true, true)
		if not valid then
			modlog(mod, reason)
		end
	end
